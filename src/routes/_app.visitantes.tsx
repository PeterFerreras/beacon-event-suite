...
import { apiClient, visitorPhotoUrl, type Visitor } from "@/lib/api";
...
function VisitorAvatar({ v }: { v: Visitor }) {
  const src = visitorPhotoUrl(v.foto, v.cedula);
  const initials = (v.nombre || "?")
    .split(/\s+/)
    .slice(0, 2)
    .map((s) => s[0]?.toUpperCase() ?? "")
    .join("");
  return (
    <div className="grid aspect-square h-12 w-12 shrink-0 place-items-center overflow-hidden rounded-full border border-cyan-300/90 bg-transparent text-xs font-semibold text-primary shadow-[0_0_12px_rgba(103,232,249,.58),0_7px_14px_-7px_rgba(15,23,42,.75)] ring-1 ring-cyan-200/70 dark:border-cyan-300/80">
      {src ? (
        <img
          src={src}
          alt={v.nombre}
...
function Visitantes() {
  ...
  const { data: visitors = [], isLoading } = useQuery({
    ...
    queryFn: () => apiClient.visitors(status),
  });
  const exitVisitor = useMutation({
    mutationFn: apiClient.exitVisitor,
    onSuccess: () => {
    ...
    () =>
      visitors.filter(
        (v) =>
          ...
          v.cedula.includes(q) ||
          v.institucion.toLowerCase().includes(q.toLowerCase()),
      ),
    [q, visitors],
  );
  return (
          ...
          <div className="mb-4 grid gap-3 sm:grid-cols-[minmax(0,1fr)_180px_140px]">
            <div className="relative">
              ...
              <Input
                ...
                placeholder="Buscar por nombre, cedula o institucion..."
                className="pl-9"
              />
            </div>
                      ...
                      <div className="flex justify-end gap-1">
                        ...
                        {v.status === "dentro" && (
                          <Button
                            size="sm"
                            variant="ghost"
                            onClick={() => exitVisitor.mutate(v.id)}
                          >
