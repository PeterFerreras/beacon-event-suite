...
import { apiClient, padronFotoUrl, type Visitor } from "@/lib/api";
import { toast } from "sonner";

export const Route = createFileRoute("/_app/visitantes")({ head: () => ({ meta: [{ title: "Visitantes - G-Visitantes" }] }), component: Visitantes });

function VisitorAvatar({ v }: { v: Visitor }) {
  const src = v.foto || padronFotoUrl(v.cedula);
  const initials = (v.nombre || "?").split(/\s+/).slice(0, 2).map((s) => s[0]?.toUpperCase() ?? "").join("");
  return (
    <div className="grid h-10 w-10 shrink-0 place-items-center overflow-hidden rounded-full border bg-primary/10 text-xs font-semibold text-primary">
      {src ? (
        <img
          src={src}
          alt={v.nombre}
...
function Visitantes() {
  ...
  return (
    <div>
      <PageHeader title="Visitantes" subtitle="Historial y estado en tiempo real de visitantes institucionales." actions={<Button asChild className="bg-accent text-accent-foreground hover:opacity-90"><Link to="/registro"><UserPlus className="mr-2 h-4 w-4" /> Registrar visitante</Link></Button>} />
        ...
        <CardContent className="p-4 sm:p-5">
          ...
          <div className="overflow-x-auto">
            <Table>
              ...
              <TableBody>
                {isLoading && <TableRow><TableCell colSpan={9} className="py-8 text-center text-sm text-muted-foreground">Cargando visitantes...</TableCell></TableRow>}
                {!isLoading && list.length === 0 && <TableRow><TableCell colSpan={9} className="py-8 text-center text-sm text-muted-foreground">No hay visitantes registrados.</TableCell></TableRow>}
                {list.map((v) => (
                  <TableRow key={v.id}>
                    <TableCell><VisitorAvatar v={v} /></TableCell>
                    <TableCell>
                    ...
                    <TableCell>{v.status === "dentro" ? <Badge className="bg-success/15 text-success hover:bg-success/15">Dentro</Badge> : <Badge variant="outline">Fuera</Badge>}</TableCell>
                    <TableCell className="text-right">
                      <div className="flex justify-end gap-1">
                        <Button size="sm" variant="ghost" onClick={() => { setBadge({ id: v.id, nombre: v.nombre, cargo: v.cargo, institucion: v.institucion, tipo: "Visitante" }); setOpen(true); }}><Printer className="h-4 w-4" /></Button>
                        {v.status === "dentro" && <Button size="sm" variant="ghost" onClick={() => exitVisitor.mutate(v.id)}><LogOut className="h-4 w-4" /></Button>}
                      </div>
                    </TableCell>
                  </TableRow>
                ))}
