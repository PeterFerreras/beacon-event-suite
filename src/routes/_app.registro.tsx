...
import { Camera, Plus, Printer, Search, Trash2, UserPlus } from "lucide-react";
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <Card className="overflow-hidden border-t-2 border-t-primary lg:col-span-2">
          ...
          <CardContent className="space-y-5">
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <div><Label>Cedula / Documento</Label><Input value={form.cedula} onChange={set("cedula")} placeholder="000-0000000-0" className="mt-1.5" /></div>
        ...
        <Card className="overflow-hidden border-t-2 border-t-accent">
          ...
          <CardContent className="space-y-4">
            <div className="grid aspect-square place-items-center rounded-[var(--radius)] border-2 border-dashed border-primary/25 bg-primary/5 text-muted-foreground">
            ...
            <div className="rounded-[var(--radius)] border border-accent/25 bg-accent/10 p-3 text-xs text-muted-foreground">
              La foto queda vinculada al historico del visitante para futuras entradas.
            </div>
            <div className="grid grid-cols-1 gap-2 pt-2">
              <Button onClick={() => register(false)}><UserPlus className="mr-2 h-4 w-4" /> Registrar entrada</Button>
              <Button className="bg-accent text-accent-foreground hover:opacity-90" onClick={() => register(true)}>
