...
function Registro() {
  ...
  const { data: sug = [] } = useQuery({
    ...
    queryFn: () => apiClient.searchVisitors(query),
  ...
  const register = (print = false) => {
    if (!form.nombre || !form.cedula) {
      toast.error("Cedula y nombre son obligatorios");
      return;
    }
        ...
        onSuccess: (visitor) => {
          if (print) {
            setBadge({
              ...
              cargo: visitor.cargo,
              institucion: visitor.institucion,
              tipo: "Visitante",
            });
            setOpen(true);
          }
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle="Registre visitantes institucionales con documento, motivo y acompanantes."
        actions={<CedulaQrScanner onScan={handleQrScan} />}
      />
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <Card className="overflow-hidden border-t-2 border-t-primary lg:col-span-2">
          <CardHeader>
            <CardTitle className="font-display">Datos del visitante</CardTitle>
          </CardHeader>
          <CardContent className="space-y-5">
            <div>
              ...
              <div className="relative mt-1.5">
                ...
                <Input
                  ...
                  placeholder="Escriba cedula o nombre..."
                  className="pl-9"
                />
                {sug.length > 0 && (
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <div>
                <Label>
                  Cedula / Documento{" "}
                  {padronLoading && <Loader2 className="ml-1 inline h-3 w-3 animate-spin" />}
                </Label>
                <Input
                  value={form.cedula}
                  onChange={set("cedula")}
              ...
              <div>
                ...
                <Input value={form.nombre} onChange={set("nombre")} className="mt-1.5" />
              </div>
              <div>
                <Label>Cargo</Label>
                <Input value={form.cargo} onChange={set("cargo")} className="mt-1.5" />
              </div>
              <div>
                <Label>Institucion</Label>
                <Input value={form.institucion} onChange={set("institucion")} className="mt-1.5" />
              </div>
              <div>
                <Label>Area a visitar</Label>
                <Input value={form.area} onChange={set("area")} className="mt-1.5" />
              </div>
              <div>
                <Label>Telefono</Label>
                <Input value={form.telefono} onChange={set("telefono")} className="mt-1.5" />
              </div>
            ...
            <div>
              <div className="flex items-center justify-between">
                ...
                <Button
                  ...
                  variant="outline"
                  onClick={() =>
                    setAcomps((a) => [...a, { id: crypto.randomUUID(), nombre: "", cedula: "" }])
                  }
                >
              ...
              <div className="mt-2 space-y-2">
                {acomps.length === 0 && (
                  <p className="text-xs text-muted-foreground">Sin acompanantes.</p>
                )}
                {acomps.map((a, i) => (
                  <div
                    key={a.id}
                  ...
                  >
                    <Input
                      placeholder={`Nombre acompanante ${i + 1}`}
                      value={a.nombre}
                      onChange={(e) =>
                        setAcomps((arr) =>
                          arr.map((x) => (x.id === a.id ? { ...x, nombre: e.target.value } : x)),
                        )
                      }
                    />
                    <Input
                      ...
                      onChange={(e) =>
                        setAcomps((arr) =>
                          arr.map((x) => (x.id === a.id ? { ...x, cedula: e.target.value } : x)),
                        )
                      }
                    />
                    <Button
                      variant="ghost"
                      size="icon"
                      onClick={() => setAcomps((arr) => arr.filter((x) => x.id !== a.id))}
                    >
          ...
          <CardContent className="space-y-4">
            ...
            <div className="grid grid-cols-1 gap-2 pt-2">
              <Button onClick={() => register(false)}>
                <UserPlus className="mr-2 h-4 w-4" /> Registrar entrada
              </Button>
              <Button
