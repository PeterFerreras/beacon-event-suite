...
function route(string $method, string $path, Database $db): void { if (padron_route($method, $path)) return;
    ...
    if (preg_match('#^/api/visitors/([^/]+)/exit$#',$path,$m) && $method==='POST') { require_role($user,[ROLE_VISITANTES]); $db->execute('UPDATE visits SET exit_at=? WHERE id=?',[now_sql(),$m[1]]); Response::json(['updated'=>true]); return; }
    ...
    if ($method==='POST' && $path==='/api/guest-types') { require_role($user,[ROLE_ADMIN]); $name=str_input(request_json(),'nombre'); if(!$name){Response::error('El nombre del tipo es obligatorio',422);return;} $id=new_id('gt_'); $db->execute('INSERT INTO guest_types (id,name,color,sort_order) VALUES (?,?,?,?)',[$id,$name,'#1d4ed8',100]); Response::json($db->one('SELECT id,name AS nombre,color FROM guest_types WHERE id=?',[$id]),201); return; }
...
function dashboard(Database $db,array $user): void { $today=date('Y-m-d'); $eventStats=['eventosActivos'=>(int)$db->one("SELECT COUNT(*) c FROM events WHERE status='activo'")['c'],'invitadosConfirmados'=>(int)$db->one("SELECT COUNT(*) c FROM guests WHERE confirmation='aceptado'")['c'],'presentesHoy'=>(int)$db->one('SELECT COUNT(*) c FROM guests WHERE DATE(arrival_at)=?',[$today])['c'],'proximosEventos'=>(int)$db->one("SELECT COUNT(*) c FROM events WHERE status<>'finalizado'")['c']]; $visitorStats=['visitantesHoy'=>(int)$db->one('SELECT COUNT(*) c FROM visits WHERE DATE(entry_at)=?',[$today])['c'],'visitantesDentro'=>(int)$db->one('SELECT COUNT(*) c FROM visits WHERE exit_at IS NULL')['c']]; if($user['role']===ROLE_EVENTOS) Response::json(array_merge(['visitantesHoy'=>0,'visitantesDentro'=>0],$eventStats)); elseif($user['role']===ROLE_VISITANTES) Response::json(array_merge($visitorStats,['eventosActivos'=>0,'invitadosConfirmados'=>0,'presentesHoy'=>0,'proximosEventos'=>0])); else Response::json(array_merge($visitorStats,$eventStats)); }
...
function users_create(Database $db): void { $d=request_json(); $name=str_input($d,'nombre'); $email=strtolower((string)str_input($d,'correo')); $role=str_input($d,'rol',ROLE_VISITANTES); $password=str_input($d,'password','Temp1234!'); if(!$name||!$email){Response::error('Nombre y correo son obligatorios',422);return;} if(!valid_user_role($role)){Response::error('Rol invalido',422);return;} $id=new_id('u_'); $db->execute('INSERT INTO users (id,name,role,email,password_hash,active) VALUES (?,?,?,?,?,1)',[$id,$name,$role,$email,password_hash($password,PASSWORD_DEFAULT)]); Response::json($db->one('SELECT id,name AS nombre,role AS rol,email AS correo,active AS activo FROM users WHERE id=?',[$id]),201); }
